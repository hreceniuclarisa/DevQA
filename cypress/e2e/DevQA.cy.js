describe('TC1', () => {
  it('Verify that all header’s elements navigate to the correct page.', () => {
    ...
    cy.get(".nav-link[href='/search']").click({ force: true })
...
describe('TC5', () => {
  it("Verify that Login functionality works with invalid credentials.", () => {
    ...
    cy.get("div[role='alert']", {force: true}).should('be.visible')
...
describe('TC8', () => {
  it("Verify that the user is able to use “Vizualizeaza” functionality",() => {
    ...
    cy.xpath("(//i[@title='Vizualizeaza'])[1]").click({ force: true })
...
describe('TC11', () => {
  it('Verify that the user is able to use “Vizualizeaza” functionality',() => {
    ...
    cy.xpath("(//i[@title='Vizualizeaza'])[25]").click()

    //    cy.get('[data-label="Status"]').contains("In progres").first().then(($row) => {
    //  const nrrow = cy.get(`//i[@title='Completeaza']`)
    //  cy.xpath(`//i[@title='Completeaza'][${nrrow}]`).click()
    //})
...
describe('TC13', () => {
  it('Verify “Completeaza” functionality',() => {
    ...
    cy.xpath("(//i[@title='Completeaza'])[1]").click({ force: true })
...
describe('TC18', () => {
  it('Verify "Contact & Sugestii" functionality on "Cazuri speciale" page', () => {
    cy.viewport(1920, 1080)
    ...
    cy.xpath("//p[normalize-space()='Cazuri speciale']").click()

    cy.get(".tim-icons.icon-simple-add").click()

    cy.xpath("//a[normalize-space()='Contact & Sugestii']").click()
...
describe('TC19', () => {
  it("Test Profile Update Functionality", () => {
    cy.viewport(1920, 1080)
    ...
    cy.get("[name='about-me']").type("descriere")

    cy.xpath("//button[normalize-space()='Salveaza']").click()

    cy.xpath("//div[@role='alert']").contains("Profilul nu a putut fi actualizat!")

  })
})

describe('TC20', () => {
  it('Test "Ofera Sugestie" Functionality', () => {
    cy.viewport(1920, 1080)

    cy.visit("https://iwanttohelp.bim.assistcloud.services/contact")

    cy.get("[name='last_name']").type("Hreceniuc Larisa")

    cy.get("[name='email']").type("larisa.hreceniuc@gmail.com")

    cy.get("[name='message']").type("Test")

    cy.xpath("//button[normalize-space()='Trimite']").click()

    cy.xpath("//div[@class='row row text-left contact-page justify-content-center']").should("be.visible")
  })
})